...
public class ControladorReunion {
	...
	//MODIFIQUE ESTO, sin validar estado
...
//	 }
	...
	public ModelAndView reunionUsuarios(@RequestParam("idReunion") Long idReunion, @ModelAttribute("sesionUsuario") Usuario us){
	  ...
	  String estado;
	  if(reunion.getUsuarios().contains(servicioLogin.usuarioporidService(us.getId()))){
		  estado = "unido";
	  }else{
		  estado = "no unido";
	  }
	...
	@RequestMapping(path="/salirDeReunion")
	public ModelAndView salirDeReunion(@ModelAttribute("sesionUsuario") Usuario usuL,@RequestParam("idReunion") Long idReunion){
		Reunion reu = servicioReunion.reunionporidService(idReunion);
		servicioReunion.salirReunionService(reu, usuL);
		return new ModelAndView("redirect:/Inicio");
	}
	
	@RequestMapping(path = "/UnionIncorrecta")
	 public ModelAndView UnionIncorrecta() {
	  return new ModelAndView("UnionIncorrecta");
	 }
}
